...
def check_dataloader(data_loader):
    for data in data_loader:
        inputs, labels = data
        print(inputs.shape, labels)  # [batchsize,samplesize,features]
...
# 针对多标签
def calculate_multi_metrics(model, data_loader):
    with torch.no_grad():
        true_labels = []
        predicted_labels = []
        for inputs, labels in data_loader:
...
def draw_loss(train_losses, vali_losses):
...
def plot_accuracy(train_accs, vali_accs):
    plt.plot(train_accs, label='Training accuracy')
    plt.plot(vali_accs, label='Test accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()
