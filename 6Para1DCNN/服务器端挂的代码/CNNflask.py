...
def make_prediction():
    input_data = request.get_json('input')                  # 获取输入
    #print(input_data)
    #input_array = json.loads(input_data)               # 将json转换为 Python 列表
    input_array = convert(input_data)
    input_tensor = torch.tensor(input_array).float()              # 将输入转换为 PyTorch 张量,并且是浮点型
    print(input_tensor.shape)
    input_tensor = input_tensor.view(1,6,100)
    print(input_tensor.shape)
    output = YXJ.predict(new_model, model_path, input_tensor) # 调用预测函数
    output = output.item()                         # 将 PyTorch 张量转换为单项float
...
def convert(input):
    ...
    for i in range(len(input)):
        accx.append(input[i]["accx"])
        accy.append(input[i]["accy"])
        accz.append(input[i]["accz"])
        gpsx.append(input[i]["gpsx"])
        gpsy.append(input[i]["gpsy"])
        gpsz.append(input[i]["gpsz"])
    output.append(accx)
    output.append(accy)
    output.append(accz)
    output.append(gpsx)
